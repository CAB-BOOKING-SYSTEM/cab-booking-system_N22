...
Microservice chịu trách nhiệm **nhận event từ RabbitMQ**, **lưu lịch sử thông báo vào MongoDB** và **phân phối thông báo real-time** đến người dùng qua **Socket.IO** (khi online) hoặc **Firebase Cloud Messaging – FCM** (khi offline).
...
- [RabbitMQ Routing Keys](#-rabbitmq-routing-keys)
- [REST API](#-rest-api)
...
┌─────────────────────────────────────────────────────────────────┐
│                      EXTERNAL INPUTS                            │
│       RabbitMQ Events         │        HTTP REST Requests       │
└──────────────┬────────────────┴────────────┬────────────────────┘
               │                             │
       ┌───────▼──────────┐       ┌──────────▼──────────┐
      │ RabbitMQ Consumer│       │  REST Controller     │
...
| `amqplib` | ^1.0.3 | RabbitMQ consumer |
...
├── package.json
├── Dockerfile
├── docker-compose.yml              # Dev stack (RabbitMQ + Redis + Service)
...
│   │
│   ├── rabbitmq/
│   │   └── consumer.js                  # RabbitMQ Infrastructure layer
│   │
│   ├── socket/
...
Service lắng nghe exchange `user.events` (type `topic`) với queue `notification_user_queue`.

| Routing Key | Loại thông báo | Người nhận | Kích hoạt bởi |
|---|---|---|---|
| `user.registered` | `SystemAlert` | **User** | User Service (đăng ký tài khoản mới) |
| `user.profile_updated` | `SystemAlert` | **User** | User Service (cập nhật hồ sơ) |
| `user.account_banned` | `SystemAlert` | **User** | User Service (khóa tài khoản) |

### Ví dụ payload `user.registered`
...
### Ví dụ payload `user.profile_updated`
...
### Ví dụ payload `user.account_banned`

```json
{
  "eventId": "evt_user_003",
  "userId": "user_556677",
  "data": {
    "reason": "Vi pham chinh sach he thong"
  }
}
```
...
|---|---|---|---|
| `cab_notification_broker_messages_processed_total` | Counter | `topic`, `status` | Tổng message đã xử lý |
...
| `cab_notification_processing_duration_seconds` | Histogram | `topic` | Latency xử lý từ broker đến lưu DB |
...
Stack bao gồm: **RabbitMQ + Redis + Notification Service**
...
| `payload` | Mixed | Metadata goc tu event RabbitMQ |
| `sourceEventId` | String (unique, sparse) | eventId de dam bao idempotency |
