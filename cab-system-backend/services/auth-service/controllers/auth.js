...
export const login = async (req, res) => {
  ...
  try {
    ...
    await safeRedisSet(`refresh:${user.id}`, refreshToken, {
      EX: 7 * 24 * 60 * 60,
    });
...
export const refresh = async (req, res) => {
  ...
  try {
    ...
    const validInDb = await hasValidRefreshTokenInDb({
      userId,
      refreshToken: oldRefreshToken,
    });
    const validByRedis = currentRefreshInRedis
      ? currentRefreshInRedis === oldRefreshToken
      : false;
    ...
    await safeRedisSet(`refresh:${user.id}`, newRefreshToken, {
      EX: 7 * 24 * 60 * 60,
    });
    await persistRefreshToken({
      userId: user.id,
  ...
  try {
    ...
    if (accessToken) {
      ...
      const accessTtl = Math.floor(
        (decodedAccess?.exp * 1000 - Date.now()) / 1000,
      );
