...
async function startServer(retryCount = 0) {
  ...
  try {
    ...
    server.listen(PORT, () => {
      logger.info(`🚗 Driver Service running on ${protocol}://localhost:${PORT}`);
      logger.info(`   WebSocket: 🔌 enabled on ${protocol === "https" ? "wss" : "ws"}://localhost:${PORT}/socket.io`);
      isRestarting = false;
    });
  } catch (error) {
  ...
  setInterval(async () => {
    try {
      ...
      if (
        ...
        database.mongoConnection.readyState !== 1
      ) {
