    ...
    for (let i = 0; i <= this.retries; i++) {
      try {
        ...
        if (i < this.retries) {
          ...
          logger.warn(`Retry ${i + 1}/${this.retries} for pricing update in ${delay}ms`);
          await new Promise(resolve => setTimeout(resolve, delay));
        }
      }
    }
    logger.warn(`⚠️ Failed to update driver count for ${zone}:`, lastError?.message);
    return null;
  }

  determineZone(lat, lng) {
    if (!lat || !lng) return 'SUBURB';
    if (lat >= 21.00 && lat <= 21.05 && lng >= 105.83 && lng <= 105.87) {
      return 'CENTER';
    }
    if (lat >= 21.20 && lat <= 21.25 && lng >= 105.78 && lng <= 105.82) {
      return 'AIRPORT';
    }
    return 'SUBURB';
  }
