...
const SignInScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const onSubmit = async (): Promise<void> => {
    ...
    setLoading(true);
    try {
      const data = await login({ email: email.trim(), password, appType: 'CUSTOMER_APP' });
      setSuccessMessage(data.message || 'Đăng nhập thành công');
      setTimeout(() => navigation.replace('Splash'), 1000);
    } catch (error) {
    ...
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <AuthCard title="Đăng nhập" subtitle="Nhập thông tin để tiếp tục">
          ...
          <TouchableOpacity style={[styles.submitBtn, loading ? styles.disabled : null]} disabled={loading} onPress={onSubmit}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitText}>Đăng nhập</Text>}
          </TouchableOpacity>
